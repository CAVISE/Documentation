...
The Python Protobuf modules used by the OpenCDA side of CAPI are generated by
the OpenCDA CMake build. Select the ``opencda-protobuf`` image target, or the
full ``opencda`` target, when OpenCDA communicates with **Artery**.
